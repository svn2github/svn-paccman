...
public class Saver extends org.paccman.xml.Saver {
    ...
    public void save(OutputStream os, DocumentController documentCtrl) throws PaccmanIOException {
        ...
        // Read title
        paccman.setTitle(document.getTitle());
        
        saveBanks(document);
        
        saveCategories(document);
        
        savePayees(document);
        
        savePaymentTypes(document);
        
        saveAccounts(document);
        
        saveTransactions(document);
        
        saveScheduledTransactions(document);
    ...
    private ReconciledStatus.Enum reconcileToXmlReconcile(TransactionBase.ReconciliationState state) throws PaccmanIOException {
        if (state == TransactionBase.ReconciliationState.UNRECONCILED   )  { return ReconciledStatus.U; } else if (state == TransactionBase.ReconciliationState.MARKED    )  { return ReconciledStatus.M; } else if (state == TransactionBase.ReconciliationState.RECONCILED)  { return ReconciledStatus.R; } else
