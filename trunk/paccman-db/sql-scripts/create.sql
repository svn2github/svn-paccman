...
CREATE TABLE REF_COUNTRIES(
    COUNTRY_ID BIGINT NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1) CONSTRAINT COUNTRIES_PK PRIMARY KEY, 
    NAME VARCHAR(100)
);
...
CREATE TABLE OBJ_TRANSACTIONS (
    ...
    -- ACCOUNT_ID may be null for template transaction only
    CONSTRAINT ACCOUNT_ID_NOTNULL CHECK((IS_TEMPLATE <> 0) OR (ACCOUNT_ID IS NOT NULL)), 
    -- VALUE_DATE may be null for template transaction only
    CONSTRAINT VALUE_DATE_NOTNULL CHECK((IS_TEMPLATE <> 0) OR (VALUE_DATE IS NOT NULL)), 
    -- TRANSACTION_DATE may be null for template transaction only
    CONSTRAINT TRANSACTION_DATE_NOTNULL CHECK((IS_TEMPLATE <> 0) OR (TRANSACTION_DATE IS NOT NULL)) 
);
